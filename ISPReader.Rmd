...
```{r}
dir="/home/geoff/ARTICLES/GRID/ISP2024/Core/"
fle="2024-ISP-Step-Change-Core.xlsx"
isp<-excel_sheets(paste0(dir,fle))
isp
```
...
getSheet<-function(name,cdp,fil,cname) {
  read_excel(paste0(dir,fle),sheet=name,skip=2) |> filter(CDP==cdp) |> normalise(cname) |> fil() 
}
nullfilter <- function(.data) {
  .data 
}
dfcsv<-read_excel(paste0(dir,fle),sheet="Storage Energy",skip=2) |> filter(CDP=="CDP3") 
...
```{r}
dfstorage<-cdp3storMWh |> filter(Technology %in% redStorTypes) |> combineStorageTypes() |> 
  mutate(year=year(ymd(Year))) |> 
  group_by(year,Region,WhoOwns) |> summarise(totalGWh=sum(GWh)) |> filter(year==2050)
write_csv(dfstorage,"storagebyowner.csv")
dfstorage<-cdp3storMWh |> filter(Technology %in% redStorTypes) |> combineStorageTypes()  |>  mutate(year=year(ymd(Year))) |> filter(year==2050)
write_csv(dfstorage,"storagebyownerall.csv")
```

```{r}
  
p<-cdp3storMWh |> filter(Technology %in% redStorTypes) |> combineStorageTypes() |> 
  ggplot()+geom_col(aes(x=Year,y=GWh,fill=WhoOwns),position="stack") +
  ...
  scale_fill_manual(values=c("grey70","grey2"),labels=c("Grid","Household"))+theme_bw()+facet_grid(.~Region)
  print(p)
...
```{r}
combineTypes<-function(.data) {
  .data |> mutate(WhoOwns=case_when(
    Technology=='Coordinated CER storage' ~ 'Household',
    Technology=='Passive CER storage' ~ 'Household',
    Technology=='Utility storage' ~ 'Grid',
    Technology=='Shallow storage' ~ 'Grid',
    Technology=='Deep storage' ~ 'Grid',
    Technology=='Medium storage' ~ 'Grid',
    Technology=='Borumba' ~ 'Borumba',
...
plotStorCap<-function(df,region) {
  p<-df |> filter(Technology %in% storCats) |> combineTypes() |> ggplot()+geom_col(aes(x=Year,y=MW,fill=WhoOwns),position="stack") +
    labs(x="",title=paste0(region," Storage capacity by year (ISP2024/CDP3)")) +
    scale_fill_manual(values=c("grey70","grey2"),labels=c("Grid","Household"))+theme_bw()
  print(p)
  dopng(paste0(region,"-CapacityISPCDP3.png"),p,w=2500,h=1800)
...
p<-cdp3cap |> filter(Technology %in% storCats) |> combineTypes() |> ggplot()+geom_col(aes(x=Year,y=MW,fill=WhoOwns),position="stack") +
  labs(x="",title=paste0("NEM"," Storage capacity by year (ISP2024/CDP3)")) +
  scale_fill_manual(values=c("grey70","grey2"),labels=c("Grid","Household"))+theme_bw()
p
dopng(paste0("NEM","-CapacityISPCDP3.png"),p,w=2500,h=1800)
```
